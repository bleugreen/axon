...
| --- | --- | --- | --- | --- | --- |
| `bbl-2026-08-16` | 2026-08-16T19:04:06-04:00 | arm64 macOS bench | Version 26.4 (Build 25E246) | granted | [`raw/bbl-2026-08-16.json`](raw/bbl-2026-08-16.json) |

## Rows

| row | target | identity | action | verdict | dispatch | target acted | arrival | invariants | controls |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `safari-click-2026-08-16` | safari (webkit-browser) | `com.apple.Safari` 26.4 | `click` | **blocked** | accepted | no | never arrived | broke ownershipProved | before: silent; after: silent |
| `safari-keyboard-2026-08-16` | safari (webkit-browser) | `com.apple.Safari` 26.4 | `keyboard` | **blocked** | accepted | no | never arrived | broke ownershipProved | before: silent; after: silent |
| `chrome-click-2026-08-16` | chrome (chromium-browser) | `com.google.Chrome` 150.0.7871.186 | `click` | **blocked** | accepted | no | never arrived | all held | before: silent; after: silent |
| `chrome-keyboard-2026-08-16` | chrome (chromium-browser) | `com.google.Chrome` 150.0.7871.186 | `keyboard` | **blocked** | accepted | no | never arrived | all held | before: silent; after: silent |
| `electron-click-2026-08-16` | electron (electron) | `com.github.Electron` 43.4.0 electron-framework None | `click` | **blocked** | accepted | no | never arrived | all held | before: silent; after: silent |
| `electron-keyboard-2026-08-16` | electron (electron) | `com.github.Electron` 43.4.0 electron-framework None | `keyboard` | **accepted** | accepted | yes | reached | all held | before: acted; after: acted |
...
| `AxonAcceptanceWebView-click-2026-08-16` | AxonAcceptanceWebView (wkwebview) | `no bundle identifier` (not readable at dispatch) | `click` | **blocked** | not observed | not observed | not observed | not established | before: not run; after: not run |
| `AxonAcceptanceWebView-keyboard-2026-08-16` | AxonAcceptanceWebView (wkwebview) | `dev.axon.acceptance.webview` 1.0.0 | `keyboard` | **blocked** | accepted | no | never arrived | all held | before: silent; after: silent |
...
- Target-side observation: the page's input read '' before and '' after
- Arrival: the page's event listeners recorded nothing
- Control before: stayed silent via `CGEventPost(kCGHIDEventTap) with the target activated` — the page's input read '' before and '' after
- Control after: stayed silent via `CGEventPost(kCGHIDEventTap) with the target activated` — the page's input read '' before and '' after
...
- Arrival: the application dequeued keyDown on window 2744, keyUp on window 2744, keyDown on window 2744, keyUp on window 2744, keyDown on window 2744, keyUp on window 2744, keyDown on window 2744, keyUp on window 2744
...
the trial could not be established: 'NoneType' object is not subscriptable

- Variant: `leftMouseDown+leftMouseUp/source=null/gapMs=0` through `CGEventPostToPid`
- Campaign: `bbl-2026-08-16`
- Fresh state: not fresh — a foreground control click preceded the measured dispatch, which is what licenses the click trial; a keyboard verdict is never taken from a trial shaped this way
- Control before: not run
- Control after: not run
