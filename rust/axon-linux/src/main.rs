...
fn linux_main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match std::env::args().nth(1).as_deref().unwrap_or("serve") {
        ...
        other => {
            return Err(
                format!("unknown subcommand {other:?}; expected serve, mcp, or shutdown").into(),
            );
        }
    }
    Ok(())
...
mod socket {
    ...
    pub fn serve(mut router: Router<LinuxBackend>) -> io::Result<()> {
        ...
        let result = (|| {
            for incoming in listener.incoming() {
                ...
                let response = if line.trim() == "{\"method\":\"shutdown\"}" {
                    ...
                    match parse_request(line.trim()) {
                        ...
                        Err(r) => serde_json::to_value(r).unwrap(),
                    }
                };
                writeln!(stream, "{}", serde_json::to_string(&response).unwrap())?;
                if line.trim() == "{\"method\":\"shutdown\"}" {
                    break;
                }
            }
            Ok(())
    ...
    fn mcp_response(value: &Value) -> io::Result<Option<Value>> {
        ...
        let response = match value.get("method").and_then(Value::as_str) {
            Some("initialize") => {
                json!({"jsonrpc":"2.0","id":id,"result":{"protocolVersion":"2025-03-26","capabilities":{"tools":{}},"serverInfo":{"name":"axon-linux","version":"0.1.0"}}})
            }
