...
For the real daemon, prefer direct `windows` crate COM bindings for Axon's narrow UI Automation surface. They are more verbose, but make COM initialization, cache requests, HRESULT handling, timeouts, and Windows features explicit. The wrapper remains useful as executable reference material, not as the architectural boundary.
...
After 500 ms, an independent recapture observed the tree grow from 61 to 91 nodes when the menu opened:

    verified_outcome=true
    verification=bounded_tree_changed before_nodes=61 after_nodes=91

This proves dispatch and observed goal progress can be represented separately. Tree-change verification is intentionally coarse; real actions need action-specific predicates and bounded waits rather than a fixed sleep.
...
The core Windows loop is viable in Rust when it runs in the interactive user session. UI Automation gives a strong native Notepad tree, locator resolution is straightforward, and Invoke dispatch can be verified separately. The implementation-shaping risks are now concrete: daemon placement must be per-user-session, and WebView2 cannot be assumed to expose a useful tree without additional work.
