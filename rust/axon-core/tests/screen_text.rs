...
fn formatter_matches_shared_screen_text_fixture() {
    ...
    for case in fixture["cases"].as_array().expect("fixture cases") {
        ...
        assert_eq!(format_screen_text(&recognized, case["frames"].as_bool().unwrap(), &Default::default()), case["expected"], "{}", case["name"].as_str().unwrap());
    }
}
...
fn active_secret_text_is_replaced_at_the_observation_boundary() {
    ...
    assert_eq!(value[0]["redaction"]["reasons"]["text"], "active-credential");
}
