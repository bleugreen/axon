...
#[derive(Deserialize)]
#[serde(rename_all = "camelCase")]
struct SemanticFixture { snapshot: Snapshot, expected: Vec<ExpectedSemanticName> }
#[derive(Deserialize)]
#[serde(rename_all = "camelCase")]
struct ExpectedSemanticName { source_index: usize, name: String, resolution: SemanticNameResolution, candidate_label: Option<String> }

#[test]
fn semantic_names_match_the_shared_language_neutral_fixture() {
    let fixture: SemanticFixture = serde_json::from_str(include_str!("../../../schema/fixtures/semantic-names.json")).unwrap();
    let actual = SemanticNameDeriver::derive(&fixture.snapshot);
    assert_eq!(actual.len(), fixture.expected.len());
    for expected in fixture.expected {
        let actual = actual.iter().find(|name| name.source_index == expected.source_index).unwrap();
