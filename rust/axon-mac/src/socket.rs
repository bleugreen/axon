...
fn answer(
    ...
    if let Err(error) = writeln!(stream, "{}", serde_json::to_string(&response).unwrap()) {
        eprintln!("axon-mac: client hung up before its answer was written: {error}");
    }
    Ok(stop)
}
...
pub fn serve() -> io::Result<()> {
    ...
    let backend = macOSBackend::start().map_err(|error| io::Error::other(error.to_string()))?;
...
fn dispatch(
    line: &str,
    router: &mut Router<macOSBackend>,
...
fn tools() -> Vec<Value> {
    ["look", "find", "invoke", "type", "scroll", "run"]
        .into_iter()
        .map(|name| {
            json!({"name":name,"description":format!("Axon macOS {name}"),"inputSchema":{"type":"object","additionalProperties":true}})
        })
        .collect()
}
