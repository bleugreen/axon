//! `axon-mac`: the AT-SPI daemon, its MCP facade, and the systemd-user lifecycle around them.
...
#[cfg(target_os = "mac")]
fn mac_main() -> Result<i32, Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str).unwrap_or("serve") {
        ...
        other => {
            eprintln!("axon-mac: unknown subcommand {other:?}\n\n{USAGE}");
...
mod lifecycle {
    ...
    pub fn daemon(subcommand: Option<&str>) -> Result<i32, Box<dyn std::error::Error>> {
        match subcommand {
            ...
            other => {
                eprintln!(
                    "axon-mac: daemon requires install, uninstall, or restart (got {other:?})"
                );
                return Ok(exit_code::USAGE);
            }
        }
    ...
    fn install() -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Some(warning) = ephemeral_path_warning(&executable) {
            eprintln!("axon-mac: warning: {warning}");
        }
    ...
    pub fn shutdown() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let stopped = match socket::shutdown_rpc() {
            ...
            Err(error) if registered => {
                eprintln!(
                    "axon-mac: the daemon did not answer a shutdown request ({error}); stopping {UNIT_NAME}"
                );
                None
            }
            Err(error) => {
...
mod status {
    ...
    use axon_mac::lifecycle::{SessionEnvironment, incompatible, not_running, registration};
    use axon_mac::socket;
    use std::io;
    ...
    pub fn run(args: &[String]) -> Result<i32, Box<dyn std::error::Error>> {
        ...
        if let Some(unexpected) = args.iter().find(|arg| *arg != "--json") {
            eprintln!("axon-mac: unexpected status argument: {unexpected}");
            return Ok(exit_code::USAGE);
        }
