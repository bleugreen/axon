...
impl Options {
    fn parse(args: impl IntoIterator<Item = String>) -> Result<Self, String> {
        ...
        if options.max_nodes == 0 {
            return Err("--max-nodes must be positive".to_owned());
        }
        if options.action
            && (options.role.is_none()
                || options.name_contains.is_none()
                || options.expect_before.is_none()
                || options.expect_after.is_none())
        {
            return Err(
                "--action requires --role, --name-contains, --expect-before, and --expect-after"
                    .to_owned(),
            );
        }
        Ok(options)
...
fn main() {
    ...
    #[cfg(target_os = "macos")]
    if let Err(error) = macos::run(&options) {
    ...
    {
        let _ = options;
        eprintln!("axon-spike-mac requires macOS");
        std::process::exit(1);
    }
}
