...
#[derive(Clone)]
struct Node { element: AXUIElementRef, depth: usize, role: String, name: String, value: String }
...
fn capture(root: AXUIElementRef, options: &Options) -> Vec<Node> {
    ...
    while let Some((element, depth)) = stack.pop() {
        ...
        let title = text_attribute(element, "AXTitle");
        let description = text_attribute(element, "AXDescription");
        let value = text_attribute(element, "AXValue");
        ...
        if let Some(children) = attribute(element, "AXChildren") {
            if unsafe { CFGetTypeID(children.0) } == unsafe { CFArrayGetTypeID() } {
                ...
                for index in (0..count).rev() {
                    ...
                    if !child.is_null() { stack.push((child, depth + 1)); }
                }
            }
        }
    }
...
pub fn run(options: &Options) -> Result<(), String> {
    ...
    let error = unsafe { AXUIElementPerformAction(target.element, action.0) };
