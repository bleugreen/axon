...
fn cfstr(value: &str) -> Result<Owned, String> {
    ...
    if value.is_null() {
        Err("CFString allocation failed".into())
    } else {
        Ok(Owned(value))
    }
}
...
pub fn run(options: &Options) -> Result<(), String> {
    ...
    let target = before
        .iter()
        .find(|node| {
            node.role.eq_ignore_ascii_case(role) && node.name.to_lowercase().contains(&name)
        })
