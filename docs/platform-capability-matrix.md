...
| I-L-WAIT | `waits_poll_captures_and_report_satisfied_and_timeout_results` proves a `Loading` → `Ready` transition across separate reads and an honest zero-timeout result; `wait_for_value_matches_every_readable_state_field` covers value, title/name, description/help, and identifier matching. | `rust/axon-linux/src/lib.rs` bounded polling routes over refreshed AT-SPI captures. | The shared facade schema advertises both waits and enforces the public duration and predicate bounds before routing. |
...
| N-W | `excluded_tools_have_structured_errors_before_backend_dispatch` covers `navigate`, `windows`, `tabs`, `permit`, `save`, and `drag`: `-32004`, capability-specific data, reason `not-implemented`; `capture_queries` remains empty. | `rust/axon-win/src/lib.rs` `EXCLUDED` guard precedes validation, capture, and backend routing. | Runtime health is irrelevant: these tools are statically absent. |
| N-L | `unimplemented_tools_have_structured_errors_before_backend_dispatch` covers `navigate`, `windows`, `tabs`, `permit`, `save`, `drag`, and `scroll`: `-32004`, capability-specific data, reason `not-implemented`; scroll dispatch count remains zero. | `rust/axon-linux/src/lib.rs` `EXCLUDED` guard precedes validation and backend routing. | Runtime health is irrelevant: these tools are statically absent. |
