...
- Done: the settle boundary is explicit and bounded on the agent path. `ActionObservationCollector` polls the target, the app's focused element, and its window titles until two consecutive reads agree, capped at a 150ms budget; a read that never agreed is marked unsettled and its string assertions are dropped rather than saved. The live recorder keeps its narrower `AXValueChanged` wait.
- Answered for the agent path: the cross-element case does not need notification inference or user direction. The observation reads the app's focused element and window titles directly, so a focus move to another element and a newly appeared window both become facts. The live recorder cannot yet feed the same compiler because `RecordedUserEventGroup` carries notification evidence rather than before/after element state; bringing it up to the same observation shape is the remaining piece.
