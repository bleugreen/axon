...
| Linux toolkit acceptance harness | `scripts/linux-toolkit-acceptance/`; results in `RESULTS.md` and `RESULTS-live-x11.md` | which toolkits act on background `XSendEvent` delivery with the session focus and real pointer unchanged, and whether AT-SPI extents match the toolkit's own rectangles; each row backed by a real-pointer and a focused-keystroke control | manual (2026-08-10, hermetic Xvfb and a live X11 session); re-run and re-date on toolkit releases |
...
Xvfb lane and again on a live Xfce X11 session, agreeing on every row: only
Chromium acts on a background click with the focus and pointer unchanged; GTK 3,
WebKitGTK, Firefox and Chromium act on background keystrokes under the same
conditions; GTK 4 receives neither; and Qt acts on both while asking to be
activated, which is not an acceptance this rung may use. GTK additionally honours
a synthetic click only while the real cursor is already inside the target window,
which is why the mechanism looks available when tried by hand.
...
the application declares about itself rather than on a window class. Implementing
it is tracked separately; the refusal stands until it is.
