...
Not yet published to an index. Build it from a checkout and depend on the directory:

```sh
pip install /path/to/axon/sdk/python
```
...
`axon/_generated.py` is generated by `sdk/generate.ts` and must not be edited. After changing the
...
python -m build     # emits an sdist and a wheel
```
