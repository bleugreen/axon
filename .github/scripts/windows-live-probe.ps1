...
try {
    ...
    & $exe daemon restart
    if ($LASTEXITCODE -ne 0) {
        throw "daemon restart failed with exit code $LASTEXITCODE"
    }

    $listRequest = '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"look","arguments":{}}}'
    $listResponse = $listRequest | & $exe mcp | ConvertFrom-Json -Depth 100
    if ($listResponse.result.isError -ne $false) {
        throw 'the app-list look request failed'
    }
    ...
    foreach ($app in $listResponse.result.structuredContent) {
        ...
        $window = $response.result.structuredContent.app.windows | ForEach-Object root | Where-Object role -eq 'Window' | Select-Object -First 1
