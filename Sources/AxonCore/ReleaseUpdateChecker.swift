...
public struct ReleaseUpdateChecker: Sendable {
    ...
    // GitHub Releases API is the authoritative, strongly-consistent source: it
    // reflects a new release immediately, before the Homebrew tap cask is
    // updated. The tap cask is kept as the *install* mechanism only.
    ...
    public static func defaultFetch(url: URL) async throws -> Data {
        // Bypass any CDN or URLCache layer so that an explicit check always
        // returns the current release, not a cached body from minutes ago.
